...
{
    std::ofstream out_file(filename); // Open the file for reading.

    // Is it actually open?
    if (!out_file.is_open()) {
        throw std::runtime_error("ERROR: Could Not Open File");
    }
    // Easiest way to grab the time/date right now.
    ...
    out_file << std::put_time(&tm, "%Y-%m-%d") << '\n';
    out_file << key << '\n';
    out_file << data;
}
